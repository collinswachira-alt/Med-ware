export interface Service {
  id: string;
  title: string;
  slug: string;
  summary: string;
  featuredImage: string;
  markdownContent: string;
}

export const services: Service[] = [
  {
    id: '1',
    title: 'Maternity Care',
    slug: 'maternity-care',
    summary: 'Comprehensive prenatal, natal, and postnatal care for expecting mothers and newborns.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119568/WhatsApp_Image_2026-02-26_at_18.08.00_1_ge5xri.jpg',
    markdownContent: `# Maternity Care at Mimosa Cottage Hospital

## Overview

We provide world-class maternity services designed to ensure the health and safety of both mother and baby throughout the pregnancy journey.

![Infant holding Mom's index finger](https://res.cloudinary.com/dwnhqho80/image/upload/v1772119579/WhatsApp_Image_2026-02-26_at_18.10.36_1_ygh3f5.jpg)

## Our Services
- **Prenatal Care**: Regular check-ups, ultrasounds, and screening tests to monitor fetal development
- **Labor & Delivery**: 24/7 obstetric care with modern delivery facilities
- **Postnatal Care**: Support for recovery and newborn care guidance
- **High-Risk Pregnancy Management**: Specialized care for complex pregnancies

## Facilities

![Mimosa staff gifting new Moms](https://res.cloudinary.com/dwnhqho80/image/upload/v1772119679/WhatsApp_Image_2026-02-26_at_18.11.17_k7howh.jpg)

- State-of-the-art delivery suites
- Neonatal Intensive Care Unit (NICU)
- Lactation consultants
- Comfortable recovery rooms

## Our Team
Our experienced obstetricians and midwives are dedicated to providing compassionate, evidence-based care at every stage of your pregnancy.

## When to Visit
- If you are pregnant or planning pregnancy
- For routine prenatal appointments
- For high-risk pregnancy monitoring
- For postpartum follow-up care

**Contact us** to schedule your maternity consultation today.`,
  },
 {
    id: '2',
    title: 'Child Wellness',
    slug: 'child-wellness',
    summary: 'Pediatric care focused on healthy growth, development, and preventive care from infancy through adolescence.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119580/WhatsApp_Image_2026-02-26_at_18.10.37_kjh5bp.jpg',
    markdownContent: `# Child Wellness at Mimosa Cottage Hospital

## Overview
We support your child’s health at every stage with preventive care, early screening, and family-centered guidance. Our goal is to help children grow, thrive, and reach their full potential through attentive, compassionate care.

![Healthy Infant](https://res.cloudinary.com/dwnhqho80/image/upload/v1772119580/WhatsApp_Image_2026-02-26_at_18.10.37_1_xtuvsc.jpg)

## Well-Child Visits & Preventive Care
- **Growth Monitoring**: Height, weight, and BMI tracking
- **Developmental Milestones**: Age-appropriate screening and counseling
- **Immunizations**: Routine and catch-up vaccines
- **Nutrition Guidance**: Feeding support from infancy to adolescence
- **Safety Counseling**: Injury prevention and safe home practices

## Clinical Services
- **Acute Illness Care**: Colds, flu, ear infections, stomach upsets
- **Chronic Condition Management**: Asthma, allergies, eczema
- **School Health Support**: Medical notes and care plans
- **Adolescent Health**: Growth counseling, nutrition, and mental well-being

## Development & Behavioral Screening
We screen for developmental delays and behavioral concerns early so families can access support quickly.

- Speech and language development
- Social and emotional development
- Learning and attention concerns
- Autism screening when indicated

## Family Education & Support
We partner with parents and caregivers, offering practical guidance on:
- Infant feeding and breastfeeding support
- Sleep routines and healthy habits
- Childhood nutrition and physical activity
- Age-appropriate safety and injury prevention

## Why Families Choose Us
- Child-friendly environment to reduce anxiety
- Evidence-based care tailored to each child
- Coordinated referrals when specialized care is needed
- Clear communication and respectful, confidential care

**Schedule your child’s wellness visit** to keep them healthy and thriving at every stage of life.`,
  },
  {
    id: '3',
    title: 'Family Planning',
    slug: 'family-planning',
    summary: 'Confidential family planning services to help you make informed reproductive health decisions.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119636/WhatsApp_Image_2026-02-26_at_18.10.38_3_bzesj5.jpg',
    markdownContent: `# Family Planning at Mimosa Cottage Hospital

## Overview

We provide confidential, non-judgmental family planning services to help you achieve your reproductive goals. Whether you're planning to start a family, space your pregnancies, or explore contraceptive options, our experienced team is here to support you with evidence-based guidance.

## Comprehensive Contraceptive Counseling

Our team provides thorough counseling on all available contraceptive methods, helping you find the option that best fits your lifestyle and health profile.

### Barrier Methods
- Condoms and diaphragms
- Non-hormonal protection
- STI prevention benefits

### Hormonal Methods
- Oral contraceptives (birth control pills)
- Transdermal patches
- Injectable contraceptives
- Flexible dosing schedules

### Long-Acting Reversible Contraceptives (LARCs)
- Intrauterine devices (IUDs)
- Contraceptive implants
- Highly effective (99%+)
- Minimal maintenance

### Permanent Methods
- Tubal ligation
- Vasectomy
- Irreversible options
- Surgical consultation and support

## Services Offered

- Comprehensive reproductive health counseling
- STI screening and testing with rapid results
- Contraceptive method selection assistance
- Fertility assessment and testing
- Pregnancy planning consultations and pre-conception counseling
- Emergency contraception (Plan B)
- Menstrual disorder management
- Sexual health education

## Our Compassionate Approach

![Negative pregnancy test](https://familymedicineaustin.com/wp-content/uploads/2022/04/9.jpg)

- Evidence-based recommendations tailored to your unique needs
- Thorough counseling on all available options with pros and cons
- Regular follow-up care and method adjustment
- Complete confidentiality and respect for your privacy
- Non-judgmental, inclusive environment
- Support for all relationship types and family structures

## Fertility Services

If you're planning to conceive:
- Fertility assessment and diagnostic testing
- Ovulation tracking and timing guidance
- Pre-conception health optimization
- Referral to fertility specialists when needed

## Who Should Visit

- Anyone seeking contraceptive advice or method change
- Couples planning pregnancy or spacing pregnancies
- Those concerned about STI exposure or sexual health
- Individuals wanting reproductive health education
- Anyone with menstrual or sexual health concerns
- Young adults seeking reproductive planning

## Why Choose Our Family Planning Services

- Experienced clinicians specializing in reproductive health
- Latest contraceptive methods and technologies
- Convenient appointment scheduling
- Affordable options for all budget levels
- Same-day prescription services
- No insurance required

**Book a confidential consultation** with our experienced family planning specialists. We're here to support your reproductive health journey.`,
  },
  {
    id: '4',
    title: 'Diagnosis & Lab Tests',
    slug: 'diagnosis-lab-tests',
    summary: 'Accurate laboratory testing and diagnostic services using state-of-the-art equipment and technology.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119574/WhatsApp_Image_2026-02-26_at_18.10.29_tpufq6.jpg',
    markdownContent: `# Diagnosis & Lab Tests at Mimosa Cottage Hospital

## Overview
We provide comprehensive laboratory and diagnostic services with timely, accurate results to support your care plan. Our team focuses on quality, safety, and clear communication so you understand every test and result.

![Diagnostic imaging consultation](https://aayush-hospitals.com/wp-content/uploads/2025/04/Diagnostic-Lab-in-Kalyan.jpg)

## Available Tests
- **Blood Tests**: Complete blood counts, chemistry panels, lipid profiles
- **Microbiology**: Bacterial and viral cultures, sensitivity testing
- **Pathology**: Tissue analysis and biopsy interpretation
- **Imaging**: X-ray, ultrasound, and advanced diagnostic imaging
- **Specialized Tests**: Hormone levels, tumor markers, genetic testing

## Core Laboratory Sections
- **Hematology**: CBCs, anemia workups, clotting studies
- **Clinical Chemistry**: Liver and kidney function, electrolytes, glucose
- **Serology & Immunology**: Infection markers and immune response tests
- **Microbiology**: Cultures, microscopy, and antimicrobial sensitivity
- **Histopathology**: Tissue processing and reporting for biopsies

## Quality Assurance
- ISO-certified laboratory
- Trained and certified technicians
- Latest diagnostic equipment
- Rigorous quality control measures
- Rapid turnaround times

## Sample Collection & Patient Comfort
- Clear, step-by-step instructions before testing
- Gentle, professional phlebotomy services
- Dedicated pediatric collection support when needed
- Privacy and confidentiality during all procedures

## Turnaround Times
Many routine tests return same day, while specialized tests are scheduled with clear timelines. We keep you informed every step of the way.

## Test Preparation
Our staff provides clear instructions for test preparation to ensure accurate results.


**Schedule your diagnostic tests** with confidence in Mimosa Cottage Hospital.`,
  },
  {
    id: '5',
    title: 'Ambulance & Referral',
    slug: 'ambulance-referral',
    summary: 'Professional ambulance services and coordinated referrals to specialized healthcare facilities.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119569/WhatsApp_Image_2026-02-26_at_18.10.23_nyyc7e.jpg',
    markdownContent: `# Ambulance & Referral Services at Mimosa Cottage Hospital

## Overview
We provide reliable ambulance services and seamless medical referrals to ensure continuity of care. Our emergency response teams are trained to stabilize patients quickly and transport them safely to the most appropriate level of care.

![Emergency ambulance ready for dispatch](https://res.cloudinary.com/dwnhqho80/image/upload/v1772201697/20250514_222114.jpg_2_dpihti.jpg)

## Ambulance Services
- **24/7 Availability**: Emergency ambulance dispatch around the clock
- **Trained Paramedics**: Certified emergency medical personnel
- **Modern Fleet**: Well-equipped ambulances with life-support equipment
- **GPS Tracking**: Real-time ambulance tracking and updates
- **Multiple Response Options**: Basic life support, advanced life support, and patient transfer

## Onboard Equipment & Capabilities
- Oxygen delivery and airway support
- Cardiac monitoring and defibrillation
- Trauma immobilization and bleeding control
- Intravenous access and emergency medications
- Neonatal and pediatric stabilization support when needed

## When to Call Our Ambulance
- Chest pain or difficulty breathing
- Severe injuries from accidents
- Sudden loss of consciousness
- Uncontrolled bleeding
- Any medical emergency requiring immediate transport

## Inter-Facility Transfers
- Safe transfer for patients needing specialized treatment
- Handover with full medical documentation
- Coordination with receiving facilities and specialists

## Referral Coordination
- Partnerships with specialized hospitals and clinics
- Coordinated patient handover
- Complete medical records transfer
- Insurance authorization assistance
- Follow-up care coordination

## What to Expect When You Call
- Rapid triage and dispatch based on urgency
- Clear instructions for bystanders and family
- Real-time updates on ambulance location
- Compassionate care throughout transport

## How to Request
Call our emergency number or use our website to request ambulance services.

**Trust us to get you to care quickly and safely.**`,
  },
  {
    id: '7',
    title: 'STIs and HIV/AIDS',
    slug: 'stis-hiv-aids',
    summary: 'Confidential testing, treatment, and management of sexually transmitted infections and HIV/AIDS.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119564/8131a353-ad73-46eb-a95c-746b759bd4d5_rlhnzb.png',
    markdownContent: `# STIs and HIV/AIDS Services at Mimosa Cottage Hospital

## Overview
We provide confidential, judgment-free testing, treatment, and long-term management of STIs and HIV/AIDS. Our goal is early detection, effective treatment, and ongoing support that protects your health and privacy.


## Services Offered
- **STI Testing**: Rapid and comprehensive screening for common STIs
- **HIV Testing**: Confidential HIV testing with rapid results
- **Post-Exposure Prophylaxis (PEP)**: Emergency treatment after potential HIV exposure
- **Pre-Exposure Prophylaxis (PrEP)**: Prevention for high-risk individuals
- **Antiretroviral Therapy (ART)**: Evidence-based HIV treatment
- **Counseling & Support**: Professional counseling and peer support groups

## Treatment & Follow-Up Care
- Same-day treatment for many STIs when indicated
- Partner notification support (with your consent)
- Follow-up testing to confirm clearance
- Adherence counseling and medication support

![Patient's specimen](https://aayush-hospitals.com/wp-content/uploads/2025/04/Diagnostic-Lab-in-Kalyan.jpg)

## Common STIs Treated
- Gonorrhea
- Chlamydia
- Syphilis
- Herpes
- HPV and genital warts
- Trichomoniasis

## HIV/AIDS Management
- CD4 count and viral load monitoring
- Opportunistic infection prevention
- Mental health support
- Nutritional counseling
- Drug adherence support

## Prevention & Education
- Safer sex education and risk-reduction counseling
- Condom access and usage guidance
- PrEP counseling for eligible clients
- PEP guidance and emergency access

## Confidentiality
Complete confidentiality is guaranteed for all patients.

## Contact Tracing
We assist with partner notification if you consent.

**Get tested today. Knowledge is power in preventing STIs and HIV.**`,
  },
  {
    id: '8',
    title: 'Breast Cancer Screening',
    slug: 'breast-cancer-screening',
    summary: 'Early detection and comprehensive breast health services including screening, imaging, and biopsy.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772203682/WhatsApp_Image_2026-02-27_at_17.38.43_yqew7l.jpg',
    markdownContent: `# Breast Cancer Screening at Mimosa Cottage Hospital

## Overview
Early detection saves lives. We offer comprehensive breast cancer screening to identify abnormalities at the earliest stages. Our dedicated team combines advanced technology with compassionate care to support your breast health journey.

## Screening Methods
- **Clinical Breast Exam**: Thorough examination by trained healthcare professionals
- **Mammography**: Digital mammography for clear imaging
- **Ultrasound**: High-resolution ultrasound for dense breast tissue
- **Advanced Imaging**: MRI for high-risk patients

## Understanding Your Breast Health
- **Self-Examination**: Learn proper technique for monthly breast self-checks
- **Symptom Recognition**: Identify lumps, dimpling, discharge, or skin changes
- **Risk Factor Assessment**: Family history, age, hormonal factors, lifestyle
- **Density Classification**: Understanding your breast tissue type and screening needs

## Risk Assessment
- Family history evaluation
- Genetic counseling and BRCA testing
- Personalized screening recommendations

## Risk Factors We Evaluate
- Age (increased risk after 40)
- Family history of breast cancer
- Personal history of breast cancer
- Hormone replacement therapy use
- Alcohol consumption and smoking
- Obesity and sedentary lifestyle
- Early menstruation or late menopause

## What to Expect
- Comfortable, private screening environment
- Experienced breast health specialists
- Quick, painless procedures
- Rapid result turnaround

![Health specialist analyzing patient results](https://res.cloudinary.com/dwnhqho80/image/upload/v1772203661/WhatsApp_Image_2026-02-27_at_17.38.42_b1tige.jpg)

## Screening Guidelines
- Women 40+: Annual mammography
- High-risk women: Earlier and more frequent screening
- Women under 40: Clinical exams and risk assessment

## Age-Based Recommendations
- **20-39 years**: Clinical breast exams every 1-3 years; learn self-examination
- **40-49 years**: Discuss screening options; begin mammography if high-risk
- **50+ years**: Annual mammography and clinical exams
- **High-risk groups**: Personalized early and frequent screening

## Abnormality Follow-up
If an abnormality is detected, we coordinate immediate specialist consultation and biopsy if needed.

## Our Comprehensive Approach
- Same-day or next-day specialist consultation for abnormal findings
- Gentle, tissue-preserving biopsy techniques when needed
- Clear communication of results and next steps
- Coordination with oncology if diagnosis of cancer is made

## Support Services
- Counseling for abnormal results
- Support groups for breast cancer survivors
- Reconstructive surgery consultations

## If Cancer Is Diagnosed
- Immediate oncology consultation
- Treatment options review (surgery, radiation, chemotherapy)
- Genetic counseling if indicated
- Psychosocial support and counseling
- Coordination with surgical and medical oncologists
- Survivorship planning and follow-up

## Community Education & Prevention
- Breast health education workshops
- Risk reduction counseling
- Lifestyle modification guidance
- Annual health awareness campaigns

**Schedule your breast cancer screening today.**`,
  },
  {
    id: '9',
    title: 'Cervical Cancer Screening',
    slug: 'cervical-cancer-screening',
    summary: 'Preventive cervical cancer screening services including Pap tests, HPV testing, and vaccination.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119565/787ba5ab-bf1b-45fb-a7fe-83d935ab7747_rcwd1h.png',
    markdownContent: `# Cervical Cancer Screening at Mimosa Cottage Hospital

## Overview
Cervical cancer is highly preventable through regular screening. We provide comprehensive cervical health services with early detection and prevention programs. Most cervical cancers develop from HPV infection, which is both preventable and treatable when caught early.

![Councelling session](https://res.cloudinary.com/dwnhqho80/image/upload/v1772204396/f93606b8-be87-4563-b059-603a6f43269c_apx2n2.png)

## Screening Services
- **Pap Test (Cervical Cytology)**: Standard cervical cancer screening using liquid-based technology
- **HPV Testing**: High-risk human papillomavirus detection for accurate risk stratification
- **Liquid-Based Cytology**: Advanced specimen collection for improved accuracy
- **Colposcopy**: Magnified visualization of cervical tissue for targeted evaluation
- **Tissue Biopsy**: When abnormalities are detected for definitive diagnosis
- **Reflex Testing**: Coordinated HPV/cytology testing for efficient follow-up

## HPV Vaccination Programs
- **Preventive Vaccination**: For women ages 9-45 years
- **Protection Coverage**: Protects against high-risk HPV strains (16, 18, 31, 33, 45, 52, 58)
- **School-Based Programs**: Coordination with educational institutions
- **Catch-Up Vaccination**: Available for women who missed routine vaccination
- **Post-Treatment**: Vaccination for women with history of abnormalities


## Comprehensive Screening Guidelines
- **Age 21-29 years**: Cytology (Pap test) every 3 years
- **Age 30-65 years**: Cytology every 3 years OR HPV testing every 5 years OR combined testing every 5 years
- **Post-Treatment**: Enhanced monitoring based on severity of abnormalities
- **Post-Hysterectomy**: Screening discontinued if cervix was removed for benign reasons
- **Immunocompromised**: More frequent screening recommended

## Risk Factors We Assess
- Early sexual activity (before age 18)
- Multiple sexual partners or partner history
- History of sexually transmitted infections
- Smoking and tobacco use
- Weakened immune system (HIV, immunosuppression)
- Long-term oral contraceptive use
- Previous abnormal Pap results or cervical procedures
- Family history of cervical cancer

## What to Expect During Screening
- **Quick Procedure**: Takes only 5-10 minutes
- **Minimal Discomfort**: Gentle speculum examination
- **Experienced Providers**: Skilled gynecologists and nurses
- **Private, Comfortable Setting**: Dedicated screening rooms
- **Clear Communication**: Full explanation of procedures and findings
- **Results Within Days**: Fast turnaround on test results

## Screening Results Explained
- **Normal (Negative)**: Continue routine screening as recommended
- **ASC-US (Atypical Squamous Cells)**: Reflex HPV testing or repeat cytology
- **LSIL (Low-Grade Lesion)**: HPV testing and possible colposcopy
- **HSIL (High-Grade Lesion)**: Colposcopy and biopsy recommended
- **AGC (Atypical Glandular Cells)**: Comprehensive evaluation needed

## Abnormal Result Management
- **Clear Communication**: Detailed explanation of results in understandable language
- **Specialist Consultations**: Immediate referral to gynecology specialists
- **Treatment Options Discussion**: Review of all available options
- **Colposcopy Guidance**: Step-by-step explanation of procedure if needed
- **Follow-up Scheduling**: Coordinated care pathway
- **Supportive Counseling**: Emotional support and reassurance

## Treatment of Cervical Abnormalities
- **LEEP/Loop Excision**: For removal of abnormal tissue
- **Cryotherapy**: Freezing abnormal cells
- **Laser Treatment**: Precise tissue removal
- **Cone Biopsy**: For further evaluation and treatment
- **Hysterectomy**: For advanced cases when indicated
- **Post-Treatment Monitoring**: Enhanced surveillance protocols

## Prevention & Education Programs
- **HPV and Cervical Health Education**: Community awareness campaigns
- **Screening Awareness**: Emphasizing importance of regular screening
- **Sexual Health Counseling**: STI prevention and safe practices
- **Smoking Cessation Support**: Tailored quit-smoking programs
- **School Education**: Reproductive health education for adolescents
- **Partner Education**: Information for male partners on HPV and role in prevention

## Support Services
- **Counseling for Abnormal Results**: Professional psychological support
- **Support Groups**: Connect with other women with cervical abnormalities
- **Survivorship Resources**: Long-term support for cancer survivors
- **Mental Health Services**: Access to counseling and psychiatric care

**Take control of your cervical health. Book your screening today.**`,
  },
  {
    id: '10',
    title: 'Immunizations',
    slug: 'immunizations',
    summary: 'Complete immunization services for infants, children, adolescents, and adults with vaccine counseling.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772119568/4b650195-2497-443d-a222-10120311e675_pn4szo.png',
    markdownContent: `# Immunizations at Mimosa Cottage Hospital

## Overview
Vaccinations are one of the most effective public health measures. We offer comprehensive immunization services for all ages, helping you and your family stay protected against preventable diseases. Our team ensures each vaccination is administered safely with proper counseling and follow-up care.

![Child after receiving vaccination](https://res.cloudinary.com/dwnhqho80/image/upload/v1772119635/WhatsApp_Image_2026-02-26_at_18.10.38_rccqoz.jpg)

## Vaccination Programs

### Childhood Vaccines (Birth - 18 years)
- **Complete Schedule**: Follows government immunization guidelines
- **Routine Immunizations**: Protection against 13+ vaccine-preventable diseases
- **Catch-Up Vaccines**: For children with delayed or incomplete schedules
- **Personalized Plans**: Customized schedules based on individual needs

### Adolescent Vaccines (9-19 years)
- **HPV Vaccination**: Protection against cervical cancer and genital warts
- **Meningococcal**: Protection against bacterial meningitis
- **Tetanus Boosters**: Maintained immunity throughout life
- **COVID-19 Series**: Age-appropriate vaccination protocols

### Adult Vaccines (20+ years)
- **Flu (Influenza)**: Annual vaccination for seasonal protection
- **Pneumococcal**: Protection against serious pneumococcal disease
- **Shingles (Herpes Zoster)**: Prevention of painful shingles infection
- **Tdap/Td Boosters**: Maintained protection throughout life
- **Hepatitis A & B**: For unvaccinated or non-immune adults

### Travel Vaccines
- **Yellow Fever**: For travel to endemic regions
- **Hepatitis A**: For travel to developing countries
- **Typhoid**: For areas with poor sanitation
- **Japanese Encephalitis**: For specific Asian destinations
- **Rabies**: For high-risk travelers
- **Pre-Travel Consultation**: Personalized recommendations based on destination

![Vaccination clinic with healthcare provider](https://images.unsplash.com/photo-1631217314830-6db1a8a8df7a?auto=format&fit=crop&w=1600&q=80)

### Occupational Vaccines
- **Healthcare Workers**: Hepatitis B, Influenza, Varicella, MMR
- **Laboratory Personnel**: Specialized protection protocols
- **High-Risk Professions**: Tailored vaccine recommendations

## Complete Vaccine List

- **DPT**: Diphtheria, Pertussis (Whooping Cough), Tetanus
- **Polio**: Inactivated poliovirus protection
- **Measles, Mumps, Rubella (MMR)**: Three-in-one viral protection
- **Varicella**: Chickenpox prevention
- **Hepatitis A**: Liver disease prevention
- **Hepatitis B**: Chronic liver infection prevention
- **Influenza (Flu)**: Seasonal respiratory protection
- **Pneumococcal**: Bacterial pneumonia and serious infection prevention
- **Meningococcal**: Meningitis prevention (multiple types available)
- **HPV**: Cervical cancer and genital wart prevention
- **Shingles (Herpes Zoster)**: Post-infectious nerve pain prevention
- **COVID-19**: Coronavirus disease prevention
- **Yellow Fever**: Tropical disease prevention (travel)
- **Rabies**: Post-exposure and pre-exposure prevention
- **Rotavirus**: Severe gastroenteritis prevention (infant)

## Age-Based Schedules

### Infants (Birth - 12 months)
- Regular visits at 2, 4, 6 months
- Protection against multiple serious diseases
- Parental education on immunization importance

### Toddlers (1-5 years)
- Booster doses to maintain immunity
- Pre-school requirements
- Catch-up schedules if needed

### School-Age Children (6-12 years)
- School entry requirements
- Adolescent preparation vaccines
- Review of complete vaccination status

### Adolescents (13-19 years)
- HPV series (if not completed earlier)
- Meningococcal vaccination
- Tetanus/Pertussis boosters
- COVID-19 vaccination

### Adults (20+ years)
- Annual flu vaccination
- Booster shots as needed
- Immunization catch-up programs
- Travel immunization consultation

## Pre-Vaccination Counseling

Our healthcare providers take time to:
- **Explain Benefits**: How each vaccine protects you
- **Discuss Risks**: Potential side effects and serious risks
- **Review Contraindications**: Medical conditions that may affect vaccination
- **Answer Questions**: Address all your concerns thoroughly
- **Assess Medical History**: Identify any special circumstances
- **Provide Written Information**: Vaccine Information Statements (VIS)
- **Review Medications**: Ensure no interactions with vaccines

## Vaccination Administration

- **Sterile Technique**: All vaccines administered with proper infection control
- **Proper Dosage**: Correct vaccine, dose, and route for each patient
- **Documentation**: Complete records of all vaccinations
- **Comfort Support**: Techniques to minimize discomfort
- **Observation Period**: Post-vaccination monitoring
- **Emergency Preparedness**: Immediate response capability if needed

## After Vaccination Care

### Expected Reactions (Normal)
- Mild fever (within 24-48 hours)
- Soreness or redness at injection site
- Mild rash (with certain vaccines)
- Temporary fatigue or mild aches
- These typically resolve within 1-2 days

### Written Guidance
- Clear instructions for home care
- When to contact healthcare provider
- Pain/fever management recommendations
- Activity and diet guidance
- Return precautions

### Post-Vaccination Monitoring
- 15-30 minute observation period for adults
- Side effect screening and documentation
- Follow-up appointment scheduling for additional doses
- Reminder system for booster schedules

### Vaccination Records
- Electronic and paper records maintained
- International vaccination certificates available
- School immunization records prepared
- Travel documentation completed
- Permanent health record maintenance

## Special Populations

### Pregnant Women
- Tailored vaccination during pregnancy
- Postpartum immunization
- Protection for newborn through antibodies

### Immunocompromised Patients
- Modified vaccine schedules
- Special considerations for effectiveness
- Personalized recommendations

### Elderly Patients (65+)
- Age-appropriate vaccine recommendations
- Enhanced protection protocols
- Regular immunization review


**Keep yourself and your loved ones protected. Schedule your vaccines today.**`,
  },
  {
    id: '11',
    title: 'General Checkups',
    slug: 'general-checkups',
    summary: 'Comprehensive physical examinations and preventive health screening for all ages.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772205317/7cc3c96e-b62c-44af-b83e-cdbfe745ba69_w9f091.png',
    markdownContent: `# General Checkups at Mimosa Cottage Hospital

## Overview
Regular health checkups are essential for maintaining good health and detecting problems early. At Mimosa Cottage Hospital, our comprehensive general checkup program is designed to identify health risks, prevent chronic diseases, and help you maintain optimal wellness throughout your life. Whether you're a young adult seeking baseline health status or a senior requiring comprehensive health assessment, our experienced physicians provide personalized evaluation and guidance.


## What's Included in a General Checkup

### Initial Assessment & History
- **Detailed Medical History**: Complete review of past illnesses, medications, and surgeries
- **Family Health History**: Assessment of hereditary conditions and genetic risks
- **Social History**: Lifestyle, occupation, stress levels, and habits
- **Symptom Review**: Discussion of any current health concerns
- **Medication Review**: Assessment of current medications and supplements
- **Allergy Documentation**: Complete allergy and adverse reaction history

### Vital Signs Measurement
- **Blood Pressure**: Hypertension screening and cardiovascular risk assessment
- **Heart Rate**: Resting and exercise heart rate evaluation
- **Body Temperature**: Baseline temperature recording
- **Respiratory Rate**: Breathing pattern assessment
- **Height & Weight**: BMI calculation and body composition assessment
- **Oxygen Saturation**: Blood oxygen levels using pulse oximetry

### Comprehensive Physical Examination
![Comprehensive health examination](https://res.cloudinary.com/dwnhqho80/image/upload/v1772205317/7cc3c96e-b62c-44af-b83e-cdbfe745ba69_w9f091.png)
Our physicians perform a systematic head-to-toe examination including:
- **Head & Neck**: Eyes, ears, nose, throat, and lymph node examination
- **Cardiovascular System**: Heart sounds and peripheral pulse assessment
- **Respiratory System**: Lung sounds and breathing pattern evaluation
- **Abdominal Examination**: Organ palpation and tenderness assessment
- **Neurological Assessment**: Reflexes, coordination, and mental status
- **Musculoskeletal System**: Joint mobility and muscle strength
- **Skin Examination**: Moles, lesions, and dermatological changes


### Organ Systems Review
- **Cardiovascular**: Heart murmurs, arrhythmias, or circulation concerns
- **Respiratory**: Cough, shortness of breath, or breathing problems
- **Gastrointestinal**: Digestive issues, appetite changes, or weight fluctuations
- **Genitourinary**: Urinary symptoms or reproductive health concerns
- **Endocrine**: Diabetes risk, thyroid function, or metabolic issues
- **Neurological**: Memory, coordination, or balance concerns
- **Mental Health**: Mood, anxiety, depression screening
- **Musculoskeletal**: Pain, stiffness, or mobility limitations

## Age-Specific Screening Programs

### Young Adults (18-40 years)
**Baseline Health Parameters**
- Establish baseline vital signs and health metrics
- Identify early risk factors for chronic disease
- Assess lifestyle and behavioral risk factors
- Screen for sexually transmitted infections if sexually active
- Psychological well-being assessment

**Recommended Screenings**
- Blood pressure monitoring
- Cholesterol screening (baseline)
- Glucose screening if risk factors present
- Blood type and Rh factor
- Mental health and substance abuse screening
- STI testing as indicated

**Preventive Health**
- Immunization review and updates
- Contraceptive counseling (as appropriate)
- Smoking cessation support
- Physical activity and nutrition guidance
- Stress management counseling

### Middle-Aged Adults (40-60 years)
**Cardiovascular Risk Assessment**
- Comprehensive lipid panel (total cholesterol, LDL, HDL, triglycerides)
- Blood pressure monitoring and hypertension management
- Diabetes screening (fasting glucose and HbA1c)
- Metabolic syndrome evaluation
- Electrocardiogram (EKG) if risk factors present

**Cancer Screening**
- Colorectal cancer screening (colonoscopy or alternative)
- Breast cancer screening (mammography for women 40+)
- Cervical cancer screening (Pap test/HPV testing for women)
- Prostate cancer discussion (PSA testing for men 45+)
- Skin cancer screening
- Lung cancer screening (smokers)

**Metabolic Screening**
- Thyroid function testing
- Liver function assessment
- Kidney function evaluation
- Bone density screening (women 50+, men 70+)

**Mental & Cognitive Health**
- Depression and anxiety screening
- Cognitive function assessment
- Sleep quality evaluation
- Stress and burnout assessment

### Senior Adults (60+ years)
**Comprehensive Geriatric Assessment**
- Fall risk evaluation and prevention strategies
- Cognitive function screening (dementia risk)
- Vision and hearing assessment
- Balance and gait evaluation
- Medication review for polypharmacy concerns
- Frailty assessment

**Chronic Disease Management**
- Hypertension management optimization
- Diabetes control and complications screening
- Heart disease risk assessment
- Kidney disease evaluation
- Liver disease screening

**Age-Specific Cancer Screening**
- Colorectal cancer (age 60-75)
- Breast cancer (mammography continuation)
- Prostate cancer (men 60-70 with life expectancy >10 years)
- Skin cancer surveillance

**Preventive Health for Seniors**
- Pneumococcal vaccination
- Annual influenza vaccination
- Shingles vaccination (if not previously received)
- Tetanus/pertussis booster
- Osteoporosis prevention and treatment
- Fall prevention strategies

## Comprehensive Laboratory Testing

### Basic Blood Work
- **Complete Blood Count (CBC)**: Red blood cells, white blood cells, platelets, hemoglobin
- **Comprehensive Metabolic Panel (CMP)**: Electrolytes, kidney function, liver function, glucose
- **Lipid Panel**: Total cholesterol, HDL, LDL, triglycerides
- **Liver Function Tests (LFTs)**: AST, ALT, alkaline phosphatase, bilirubin
- **Kidney Function Tests**: Creatinine, BUN, eGFR
- **Thyroid Function**: TSH and T4 levels (if indicated)
- **Fasting Glucose**: Diabetes screening
- **HbA1c**: Average blood sugar over 3 months (diabetes assessment)

### Specialized Testing
- **Hepatitis B & C Screening**: Viral infection assessment
- **HIV Testing**: With consent and counseling
- **Syphilis Testing**: Serological screening
- **Urinalysis**: Kidney and urinary tract assessment
- **Prostate-Specific Antigen (PSA)**: For men 45+ if desired
- **Tumor Markers**: If family history of specific cancers

### Additional Tests (When Indicated)
- **EKG/ECG**: Baseline cardiac rhythm assessment
- **Chest X-ray**: Lung and cardiac imaging
- **Ultrasound**: Abdominal or pelvic imaging if needed
- **Blood Pressure Monitoring**: 24-hour ambulatory BP if hypertension suspected

## Health Counseling & Personalized Guidance

### Nutrition & Diet
- **Dietary Assessment**: Current eating habits and nutritional status
- **Personalized Recommendations**: Based on age, health status, and goals
- **Mediterranean Diet**: Heart-healthy eating patterns
- **Weight Management**: Healthy weight maintenance or loss strategies
- **Food Safety**: Proper food handling and preparation
- **Nutritionist Referral**: For complex dietary needs

### Physical Activity & Fitness
- **Exercise Assessment**: Current activity level and fitness
- **Age-Appropriate Recommendations**: Safe exercise guidelines
- **Cardiovascular Exercise**: 150 minutes moderate activity weekly
- **Strength Training**: 2 sessions per week for muscle maintenance
- **Flexibility & Balance**: Especially important for seniors
- **Fitness Program Referral**: Access to gym or physical therapy services

### Mental Health & Stress Management
- **Stress Screening**: Assessment of psychological stressors
- **Anxiety & Depression Screening**: Mental health evaluation
- **Sleep Assessment**: Sleep quality and sleep disorders
- **Stress Reduction Techniques**: Meditation, deep breathing, mindfulness
- **Mental Health Referral**: Counseling or psychiatric services if needed
- **Work-Life Balance**: Practical strategies for wellness

### Lifestyle Modifications
- **Smoking Cessation**: Support programs and nicotine replacement
- **Alcohol Reduction**: Safe drinking guidelines and support
- **Sleep Hygiene**: Tips for better sleep quality
- **Social Connection**: Importance of relationships and community
- **Time Management**: Practical strategies for reducing stress

## Disease Prevention & Screening Recommendations

### Cardiovascular Disease Prevention
- Blood pressure target: <120/80 mmHg for most adults
- Cholesterol management strategies
- Regular aerobic exercise
- Heart-healthy diet
- Weight management
- Stress reduction

### Diabetes Prevention
- Weight management if overweight
- Increased physical activity
- Healthy eating patterns
- Regular glucose monitoring
- Annual screening for at-risk individuals

### Cancer Prevention & Early Detection
- Smoking cessation critical for lung cancer prevention
- Sun protection and skin self-examination
- Colorectal cancer screening as recommended
- Breast and cervical cancer screening for women
- Prostate cancer discussion for men

### Osteoporosis Prevention
- Calcium and vitamin D intake assessment
- Weight-bearing exercise
- Bone density screening when indicated
- Medication review
- Fall prevention strategies

## Vaccination & Immunization Review

- **Tetanus & Pertussis (Tdap/Td)**: Booster every 10 years
- **Influenza (Flu)**: Annual vaccination
- **Pneumococcal**: Age and risk-based recommendations
- **Shingles (Herpes Zoster)**: Recommended at age 50+
- **Hepatitis A & B**: For unvaccinated or at-risk individuals
- **Measles, Mumps, Rubella (MMR)**: If immunity status uncertain
- **COVID-19**: Current vaccination status assessment

## Benefits of Regular General Checkups

### Early Disease Detection
- Identify diseases before symptoms develop
- Improved treatment outcomes with early intervention
- Prevention of serious complications
- Extended life expectancy

### Chronic Disease Management
- Improved control of diabetes, hypertension, and heart disease
- Regular monitoring and medication adjustment
- Prevention of disease progression
- Better quality of life

### Preventive Health
- Reduced risk of heart disease, stroke, and cancer
- Healthier lifestyle habits
- Fewer emergency hospitalizations
- Lower overall healthcare costs

### Health Goal Achievement
- Weight management success
- Fitness and wellness goals
- Mental health improvement
- Sustained lifestyle changes

### Peace of Mind
- Reassurance from comprehensive health assessment
- Clear understanding of health status
- Action plan for wellness
- Reduced health anxiety

## Appointment Details & What to Bring

### Before Your Appointment
- List of current medications and supplements
- Insurance card and photo ID
- Medical records from other providers if available
- Notes on any health concerns
- Fasting 8-10 hours if blood work planned

### What to Expect
- Appointment duration: 45-60 minutes
- Private examination room
- Same-day or next-day test results for most screenings
- Written summary of findings and recommendations
- Follow-up appointment scheduling if needed

### After Your Appointment
- Detailed written report of findings
- Personalized health recommendations
- Prescription for any needed medications
- Referrals to specialists if indicated
- Clear explanation of any abnormal results

## Our Comprehensive Approach

- **Patient-Centered Care**: Your health goals guide our recommendations
- **Evidence-Based Practice**: Latest medical guidelines inform our screening
- **Accessible Follow-Up**: Easy scheduling for any recommended repeat visits
- **Health Education**: Understanding your results and prevention strategies
- **Preventive Focus**: Keeping you healthy is our priority

**Schedule your comprehensive general checkup today and take control of your health.**`,
  },
  {
    id: '12',
    title: 'In & Out Patient Services',
    slug: 'in-out-patient-services',
    summary: 'Inpatient hospital care and outpatient treatment services with comfortable facilities and expert medical teams.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772205739/Mimosa-Cottage-Hospital-Nairobi-Kenya-AskAnnie-TortoisePathcom-4-1024x578_dzxsct.jpg',
    markdownContent: `# In & Out Patient Services at Mimosa Cottage Hospital

## Overview
We provide comprehensive inpatient and outpatient services to meet all your healthcare needs. Whether you need same-day care, specialist consultation, or hospital admission, our multidisciplinary teams deliver safe, coordinated treatment in a comfortable environment.


## Inpatient Services

### Medical & Surgical Admissions
- **General Medicine**: Care for infections, chronic disease flare-ups, and complex medical conditions
- **Surgical Care**: Pre-op evaluation, surgery, and post-op recovery
- **Maternity & Postnatal**: Safe delivery support and recovery care
- **Pediatric Admissions**: Child-friendly monitoring and treatment

### Critical Care Units
- **ICU Beds**: Advanced life support and continuous monitoring
- **High-Dependency Unit (HDU)**: Close observation for patients needing extra support
- **Emergency Stabilization**: Rapid assessment and escalation when necessary

### Inpatient Diagnostics & Monitoring
- 24/7 nursing assessment and vital sign monitoring
- Bedside laboratory and imaging when needed
- Medication administration and safety checks
- Multidisciplinary rounds with physicians, nurses, and pharmacists

## Inpatient Amenities & Comfort
- Private and semi-private rooms
- 24/7 nursing care and daily physician visits
- Modern medical equipment and safety monitoring
- Family visitation areas and patient support
- Patient entertainment (TV, Wi‑Fi)
- Nutritious meals and dietary plans
- Pain management and comfort protocols


## Outpatient Services

### Specialist Clinics
- **Internal Medicine**: Chronic disease management and follow-ups
- **Cardiology**: Heart health assessment and monitoring
- **Orthopedics**: Joint, bone, and sports injury care
- **Dermatology**: Skin, hair, and nail conditions
- **ENT & Eye Care**: Ear, nose, throat, and vision services

### Diagnostic & Treatment Services
- On-site lab testing and imaging
- Minor surgery and day procedures
- Wound care and dressing changes
- IV therapy and injections
- Physiotherapy and rehabilitation
- Nutrition counseling and lifestyle coaching

### Mental Health & Wellness
- Counseling services for stress, anxiety, and depression
- Behavioral health screening and support
- Referral pathways for specialized psychiatric care

![Inpatient accomodation facility](https://res.cloudinary.com/dwnhqho80/image/upload/v1772205757/jjj_vrocvt.jpg)

## Outpatient Day Procedures
- Safe, scheduled procedures with same-day discharge
- Pre-procedure evaluation and consent
- Post-procedure monitoring and discharge instructions
- Clear follow-up plan and review appointments

## Admission Process (Inpatient)
- Rapid triage and admission based on clinical need
- Clear explanation of diagnosis and care plan
- Consent and documentation support
- Family guidance on visitation and updates

## Discharge & Continuity of Care
- Discharge summary and medication instructions
- Follow-up appointment scheduling
- Referrals to specialists when needed
- Home care guidance and red-flag symptoms to watch for

## Insurance & Billing
- Most major insurance plans accepted
- Transparent billing and cost estimates
- Support with authorization and claims
- Cash and flexible payment options available

## Why Choose Our In & Out Patient Services
- Experienced clinicians and compassionate nursing care
- Coordinated care between inpatient and outpatient teams
- Modern facilities and diagnostic support
- Patient-centered, respectful approach
- Clear communication and prompt follow-ups

**Whether you need inpatient or outpatient care, we’re here for you with safe, compassionate service.**`,
  },
  {
    id: '13',
    title: 'Blood Transfusion',
    slug: 'blood-transfusion',
    summary: 'Blood bank services including donation, testing, storage, and transfusion of safe, screened blood products.',
    featuredImage: 'https://res.cloudinary.com/dwnhqho80/image/upload/v1772206289/572a90c0-4ced-4332-be79-ab7bd03f8970_eujnbh.png',
    markdownContent: `# Blood Transfusion Services at Mimosa Cottage Hospital

## Overview
Our blood bank ensures a safe and reliable blood supply for patients in need. We provide comprehensive donation, screening, storage, and transfusion services using strict quality standards to protect donors and recipients.


## Blood Donation Services

### Types of Donations
- **Whole Blood Donation**: Standard donation used for many patients
- **Plasma Donation**: Plasma used for clotting and immune support
- **Platelet Donation**: Critical for cancer patients and those with bleeding disorders
- **Directed Donation**: For patients requesting blood from a specific donor
- **Mobile Blood Drives**: Community outreach to increase supply

### Donor Eligibility Requirements
- Age 18-65 years (or per medical assessment)
- Minimum weight 50 kg
- Good general health
- No recent infections or vaccinations
- Normal hemoglobin levels
- Free and confidential screening at time of donation

### Donor Safety & Comfort
- Pre-donation health assessment and counseling
- Sterile, single-use collection equipment
- Comfortable donation area with trained staff
- Refreshments and recovery monitoring post-donation
- Donor education on recovery and next donation timing


## Blood Testing & Safety Screening

### Mandatory Screening
- HIV 1 & 2
- Hepatitis B and C
- Syphilis testing
- Malaria screening (as indicated)
- Blood grouping and cross-matching

### Quality & Safety Standards
- Rigorous donor history screening
- Multiple disease testing before release
- Quality control at every step of processing
- Complete traceability from donor to recipient
- Safe storage and cold chain management

## Blood Types & Inventory
![Blood donation ](https://res.cloudinary.com/dwnhqho80/image/upload/v1772206295/071c62dc-4cdd-47a9-9510-b4e7df547c91_y6egj9.png)
We maintain supplies of all major blood types:
- O+, O-, A+, A-, B+, B-, AB+, AB-
- Emergency priority for O-negative (universal donor)
- Special preparations for rare blood groups

## Transfusion Services

### Emergency Transfusions
- Rapid provision for trauma, obstetric emergencies, and critical bleeding
- Immediate cross-match and compatibility testing
- 24/7 availability for urgent cases

### Planned & Chronic Transfusions
- Scheduled transfusions for surgical patients
- Support for chronic anemia or blood disorders
- Oncology and hematology patients
- Preoperative optimization for elective surgery

### Blood Components Available
- **Packed Red Cells**: For anemia and blood loss
- **Fresh Frozen Plasma (FFP)**: For clotting factor support
- **Platelets**: For low platelet counts
- **Cryoprecipitate**: For fibrinogen replacement

## Transfusion Process & Monitoring
- Pre-transfusion clinical assessment
- Informed consent and counseling
- Vital sign monitoring during transfusion
- Immediate response to any adverse reactions
- Post-transfusion evaluation and documentation


## Patient Safety & Risk Management
- Compatibility testing for every transfusion
- Strict identification and labeling procedures
- Adverse reaction protocols and emergency readiness
- Continuous audit and staff training
- Infection control and sterile handling

## Benefits of Blood Donation
- Save lives in emergencies and surgeries
- Free health screening and basic tests
- Community impact and recognition
- Safe and supervised donation process

## Become a Donor
Register as a blood donor and make a difference in your community. Our team will guide you through the process and ensure a safe, comfortable experience.

**Donate blood. Save lives. Be a hero.**`,
  },
];

// Helper function to get service by slug
export function getServiceBySlug(slug: string): Service | undefined {
  return services.find((service) => service.slug === slug);
}

// Helper function to get related services (exclude current service)
export function getRelatedServices(currentSlug: string, limit: number = 4): Service[] {
  return services
    .filter((service) => service.slug !== currentSlug)
    .slice(0, limit);
}
